...
export async function send_code(email: string): Promise<{ success: boolean; code: string; error?: string }> {
  ...
  try {
    ...
    return { success: false, code: "", error: `Database error: ${(err as Error).message}` };
  }
}

// Verifies a user's email and code, deletes the code after use
export async function verify(email: string, code: string): Promise<{ success: boolean; error?: string }> {
  try {
...
export async function get_name_and_cell(email: string, name: string, cell: string): Promise<{ success: boolean; error?: string }> {
  try {
...
export async function save_user(name: string, cell: string, email: string): Promise<{ success: boolean; error?: string }> {
  try {
    if (!/^\d{3}-\d{3}-\d{4}$/.test(cell)) {
      throw new Error("Invalid phone number format. Use XXX-XXX-XXXX format.");
    }
