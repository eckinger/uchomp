...
export default function ViewGroups() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const filteredOrders = response.filter((order: Order) => order.loc === selectedLocation);
  ...
  const formatTimeRemaining = (expirationDate: string): string => {
    ...
    const now = new Date();
    const minutesRemaining = Math.floor((expiration.getTime() - now.getTime()) / 60000);
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="flex items-center text-gray-600">
                    ...
                    <span>{order.participants ? order.participants.length : 0}</span>
                  </div>
                </div>
                <div className="flex items-center text-gray-600 mb-3">
                  <Clock size={16} className="mr-1" />
                  {parseInt(formatTimeRemaining(order.expiration)) < 60 ? (
                    <span>Ordering in {formatTimeRemaining(order.expiration)}</span>
                  ) : (
                    <span>Ordering at {formatTimeRemaining(order.expiration)}</span>
                  )}
                </div>
